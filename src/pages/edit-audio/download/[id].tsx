...
import { removeSpecialCharacters, downloadFile } from "@/utils/utils";
  ...
  const init = async () => {
    try {
      ...
      if (!localFile && !localFileQuickBlob) {
        ...
        arr.pop();
        let blob = localFile?.arrayBufferBlob || localFileQuickBlob?.arrayBufferBlob;
        console.log("arraybuffer", blob);
        blob = arrayBufferToBlob(blob);
        console.log("blob", blob);
        console.log(blob, name, "audio/ogg");
        downloadFile(blob, name, "audio/ogg");
      }
    } catch (e) {
      console.log(e);
      toast(c("genericErrorMessage"), "error");
